...
##################################### Driver Unsuspention Action ####################################
class DriverUnsuspentionAction(ImportCsvInterface):
    def __init__(self,
                 df_name,
                 df,
                 token,
        ...
        self._df_cols = [{'col': 'driver_uid', 'type': 'str'}]
    ...
    def run(self):
        ...
        print('Running DriverUnsuspentionAction ...')
        self.validate_dataset()
        self.post_data()

    def post_request(self, row):

        uri = env.api_url + env.driver_unsuspension_endpoint.format(row['driver_uid'])
